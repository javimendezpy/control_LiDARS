...
def leer_datos_origen(informe_mtto, hoja_mtto):
    ...
    return n_equipo, fecha_ultimomantenimiento, comentario_ultimomantenimiento, metanol_ultimomantenimiento,\
          liquido_ultimomantenimiento, codigo_incidencias , sensores_cambiados_ultimomantenimiento, estado_baterias



def leer_correos_origen(informe_mtto, hoja_mtto):
    """
    Lee del archivo orgien:
    las filas de la columna 18 con los correos a los que se debe enviar el informe 
    y la columna 17 con la fecha y hora en fomato dd-mm-yy hh:mm a la que se debe enviar el correo
    """
    ...
    fecha_envio = df.iloc[0, 17]  # Obtiene valor fecha de envio del correo
    correos_destino = df.iloc[0:, 18].dropna().tolist()  # Obtiene valor correos destino
    print(correos_destino, type(correos_destino))

    # Comprobar el formato de los datos leídos:
    ...
    assert isinstance(fecha_envio, (datetime.datetime)), \
    f"fecha_envio debe ser datetime o Timestamp, no {type(fecha_envio)}"

    print("Correos y fechas leídos correctamente del archivo de origen.")
...
def actualizar_fecha_destino(informe_destino, n_equipo, fecha_ultimomantenimiento,
    ...
    fecha_penult_str = fecha_penultimomantenimiento.strftime("%d/%m/%Y") if isinstance(fecha_penultimomantenimiento, datetime.datetime) else str(fecha_penultimomantenimiento)
    fecha_ult_str = fecha_ultimomantenimiento.strftime("%d/%m/%Y") if isinstance(fecha_ultimomantenimiento, datetime.datetime) else str(fecha_ultimomantenimiento)
