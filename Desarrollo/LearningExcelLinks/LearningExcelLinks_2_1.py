...
def leer_datos_origen(informe_mtto, hoja_mtto):
    ...
    return n_equipo, fecha_ultimomantenimiento


#Actualizar el archivo destino con los datos leídos del archivo de origen
def actualizar_datos_destino(informe_destino, n_equipo, fecha_ultimomantenimiento, hoja_destino_name='Lidar Windcube', hoja_destino_historico_name='Historico'):
    """
    Actualiza el archivo destino con los datos del archivo de origen.
    """
    ...
    for idx, row in enumerate(hoja_destino.iter_rows(min_row=3, max_col=1, values_only=True), start=3):
        if row[0] == n_equipo:
            # Accede fecha del penultimo mantenimiento
            ...
            current_value = hoja_destino_historico.cell(row=idx, column=2).value
            if current_value:
                # Concatenate as comma-separated string
                new_value = f"{current_value},{fecha_ultimomantenimiento}"
            else:
                # If no previous value, start with the last maintenance date
                new_value = str(fecha_ultimomantenimiento)

            hoja_destino_historico.cell(row=idx, column=2, value=new_value)
            break
...
def main():
    ...
    n_equipo, fecha_ultimomantenimiento = leer_datos_origen(informe_mtto, hoja_mtto)
    
    # Actualizar el archivo destino con los datos leídos
    actualizar_datos_destino(informe_destino, n_equipo, fecha_ultimomantenimiento, hoja_destino, hoja_destino_historico)
    actualizar_datos_destino(informe_destino, n_equipo, fecha_ultimomantenimiento)
