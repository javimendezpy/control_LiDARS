...
def leer_datos_origen(informe_mtto, hoja_mtto):
    ...
    metanol_ultimomantenimiento = [df.iloc[0, 6] , df.iloc[0, 7], df.iloc[0, 8]]  # Obtiene valor metanol del ultimo mantenimiento [Cambio metanol (booleano), Cartuchos restantes (int)]
    ...
    assert isinstance(metanol_ultimomantenimiento[2],(int, np.integer)), f"Cartuchos añadidos al STOCK debe ser int o float, no {type(metanol_ultimomantenimiento[1])}"

    # assert isinstance(metanol_ultimomantenimiento[2],(int, float, np.integer, np.floating)), f"Cartuchos añadidos al STOCK debe ser int o float, no {type(metanol_ultimomantenimiento[1])}"
...
def actualizar_metanol_destino(informe_destino, n_equipo, metanol_ultimomantenimiento, hoja_destino_name='Lidar Windcube', hoja_destino_historico_name='Historico'):
    """
    Actualiza el archivo destino con el metanol del archivo de origen.
    """
    # Cargar el libro de trabajo
    wb = load_workbook(informe_destino)
    hoja_destino = wb[hoja_destino_name]
    ...
    if cambio_metanol:  # Si se ha cambiado el metanol
        for idx, row in enumerate(hoja_destino.iter_rows(min_row=3, max_col=1, values_only=True), start=3):
            if row[0] == n_equipo:
                ...
                # Actualizar la columna 5 con el valor de cartuchos de metanol gastados en la historia del LiDAR (= Cartuchos totale instalados en el EFOY)
...
def main():
    ...
    n_equipo, fecha_ultimomantenimiento, comentario_ultimomantenimiento, metanol_ultimomantenimiento = leer_datos_origen(informe_mtto, hoja_mtto)
    ...
    # Actulizar el archivo destino con las nuevas medidas de metanol despues el mantenimiento
