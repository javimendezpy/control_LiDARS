...
n_equipo = df_origen.iloc[0, 0] #Obtiene valor n equipo LiDAR
...
hoja_destino_historico = wb['Historico']


# Buscar la fila donde la primera columna coincide con n_equipo LiDAR,
# copiar la fecha de ultimo mantenimiento desde el archivo de origen y hacer append en la columna 1 de la hoja 'Historico',
...
for idx, row in enumerate(hoja_destino.iter_rows(min_row=3, max_col=1, values_only=True), start=3):
    if row[0] == n_equipo:
        ...
        if current_value:
            ...
            new_value = str(fecha_penultimomantenimiento)
