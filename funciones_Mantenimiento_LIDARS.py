...
def encontrar_fila_destino(wb, n_equipo, hoja_destino_name):
    """
...
def encontrar_fila_historico(n_equipo, ubicacion, carpeta_destino_historico, plantilla_historico, hoja_plantilla_historico, informe_destino, hoja_destino_name):
    ...
    if not os.path.exists(excel_historico):
        ...
        client = input(f"Introduce el cliente (en mayúsculas) asociado al LIDAR {n_equipo}: ").strip()
    ...
    # Caso 2: Ya existe el Excel
        ...
        # Caso 2b: no existe la hoja → preguntar si se crea hoja de nueva ubicación
        else:
            while True:
                ...
                if cambio_ubicacion == "s":
                    ...
                    # Escribir nombre de la nueva ubicación en el Excel Padre:
                    wb_padre = load_workbook(informe_destino)
                    hoja_padre = wb_padre[hoja_destino_name]
                    fila_padre = encontrar_fila_destino(wb_padre, n_equipo, hoja_destino_name)
                    hoja_padre.cell(row=fila_padre, column=2, value=ubicacion)
                    wb_padre.save(informe_destino)
...
def actualizar_fecha_destino(informe_destino, n_equipo, fecha_ultimomantenimiento, fila_hist,
                             ...
                             hoja_destino_name="Lidars"):
    """
    Actualiza la fecha del último mantenimiento en:
    1. El Excel padre (hoja 'Lidars').
    ...
    Parámetros:
        ...
        hoja_destino_name (str): Nombre de la hoja del Excel padre (default: 'Lidars').
    """
    wb_padre = None
    wb_hist = None
...
def actualizar_comentario_destino(informe_destino, n_equipo, comentario_ultimomantenimiento, fila_hist,
                             carpeta_destino_historico, ubicacion, hoja_destino_name="Lidars"):
    """
...
def actualizar_extintor_destino(n_equipo, codigo_incidencias, fila_hist, carpeta_destino_historico, ubicacion):
    ...
    if fecha_caducidad is None:
        ...
        print("✔ No se actualiza extintor porque no existe en este equipo.")
        return
...
def actualizar_sensores_destino(informe_destino, n_equipo, codigo_incidencias, fila_hist, carpeta_destino_historico, ubicacion, hoja_destino_name='Lidars'):
    """
